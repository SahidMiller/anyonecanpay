...
export default class TransactionBuilder {
   ...
   */
  static addOutput(satoshis:number, address: string): { value: Buffer, locking_script:Buffer } {
    ...
    // Check if the provided satoshis is too large to be accepted.
    if (!locking_script) {
      throw `Cannot add output, unable to parse locking script from address`;
    }

    // Structure the output
    return {
      value: TransactionBuilder.encodeOutputValue(satoshis),
      locking_script: (<Buffer>locking_script),
    };
  }
   ...
   */
  static encodeOutputValue(satoshis:number):Buffer {
    // Check if the provided satoshis is of the correct type.
    if (isNaN(satoshis)) {
   ...
   *
   */
  static decodeOutputValue(value:Buffer):number {
  ...
  static getLockscriptFromAddress(address:string):boolean|Buffer {
    // Check if the provided address is properly encoded.
    if (!Address.isCashAddress(address)) {
    ...
    // Detect address type.
    const type = Address.detectAddressType(address);
  ...
  static parseKeyHashUnlockScript(unlockScript:Uint8Array):{publicKey:Uint8Array, signature:Uint8Array} {
  ...
  static encodeOutputIndex(index:number):Buffer {
